...
class DataParcelable(val elements: Elements) {
   ...
   */
  fun createParcelableWrapper(typeElement: TypeElement): String {
    val typeName = ClassName.get(typeElement)
    val file = KotlinFile.builder(typeName.packageName(), "Parcelable${typeName.simpleName()}")
    file.apply {
      val properties = typeElement.getFields(elements).map {
        PropertySpec.builder(it.simpleName.toString(), TypeName.get(it.asType())).build()
      }
      ...
      val wrapper = TypeSpec.classBuilder(wrapperClassName).apply {
        ...
        addFun(FunSpec.builder("unparcel").apply {
          returns(typeName)
      ...
      addFun(FunSpec.builder("read${typeName.simpleName()}").apply {
        ...
        addStatement("return p.unparcel()")
      }.build())
